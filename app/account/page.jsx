...
function AccountPageInner() {
  ...
  const handleRedirectAfterAuth = async (user) => {
    ...
    router.push(profile.role === 'landlord' ? '/dashboard' : '/my-account')
  }
  ...
  const handleOnboardingSubmit = async (e) => {
    e.preventDefault()
    if (!onboardingRole) { setError('Please select your role.'); return }
    if (!onboardingPhone.trim() || !onboardingPhone.match(/^\+?[0-9]{10,14}$/)) {
    ...
    const { error } = await supabase
      .from('Profiles')
      .update({ role: onboardingRole, phone: onboardingPhone })
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#f5f4f0', fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <div className="faim-account-card">
        ...
        {mode === 'onboarding' && (
          <form onSubmit={handleOnboardingSubmit} className="faim-form" style={{ marginTop: '0.5rem' }}>
            <div style={{ textAlign: 'center', marginBottom: '1rem' }}>
              <div style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>👋</div>
              <h2 style={{ fontSize: '1.1rem', fontWeight: 700, color: '#1a1a2e', margin: '0 0 0.25rem' }}>
                Welcome{onboardingUser?.full_name ? `, ${onboardingUser.full_name.split(' ')[0]}` : ''}!
              </h2>
              <p style={{ fontSize: '0.85rem', color: '#666', margin: 0 }}>Just two quick things to get you started.</p>
            </div>
            <div className="faim-field">
              <label>I am a</label>
              <div className="faim-role-toggle">
                <button
                  type="button"
                  className={`faim-role-btn ${onboardingRole === 'tenant' ? 'faim-role-btn--active' : ''}`}
                  onClick={() => setOnboardingRole('tenant')}
                >
                  🔍 Tenant
                </button>
                <button
                  type="button"
                  className={`faim-role-btn ${onboardingRole === 'landlord' ? 'faim-role-btn--active' : ''}`}
                  onClick={() => setOnboardingRole('landlord')}
                >
                  🏠 Landlord
                </button>
              </div>
            ...
            <button type="submit" className="faim-submit-btn" disabled={loading}>
              {loading ? 'Saving...' : 'Continue to my account →'}
            </button>
          </form>
        )}
