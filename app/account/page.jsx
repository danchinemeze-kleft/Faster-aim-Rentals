    ...
    <main className="faim-account-page">
      <div className="faim-account-card">
        ...
        <div className="faim-tab-toggle">
          <button className={`faim-tab ${mode === 'login' ? 'faim-tab--active' : ''}`}
            onClick={() => { setMode('login'); setError(''); setSuccessMsg('') }} type="button">
            Log in
          </button>
          <button className={`faim-tab ${mode === 'signup' ? 'faim-tab--active' : ''}`}
            onClick={() => { setMode('signup'); setError(''); setSuccessMsg('') }} type="button">
            Sign up
          </button>
        </div>
        ...
        <button className="faim-google-btn" onClick={handleGoogleLogin} disabled={googleLoading} type="button">
        ...
        {mode === 'signup' && (
          <form onSubmit={handleSignup} className="faim-form">
            <div className="faim-field">
              ...
              <input id="signup-name" type="text" placeholder="Daniel Igboke"
                value={fullName} onChange={e => setFullName(e.target.value)} required autoComplete="name" />
            </div>
            <div className="faim-field">
              <label htmlFor="signup-phone">Phone number</label>
              <input id="signup-phone" type="tel" placeholder="080XXXXXXXX"
            ...
            <div className="faim-field">
              ...
              <div className="faim-role-toggle">
                <button type="button" className={`faim-role-btn ${role === 'tenant' ? 'faim-role-btn--active' : ''}`}
                  onClick={() => setRole('tenant')}>🔍 Tenant</button>
                <button type="button" className={`faim-role-btn ${role === 'landlord' ? 'faim-role-btn--active' : ''}`}
                  onClick={() => setRole('landlord')}>🏠 Landlord</button>
              </div>
            </div>
            <button type="submit" className="faim-submit-btn" disabled={loading}>
