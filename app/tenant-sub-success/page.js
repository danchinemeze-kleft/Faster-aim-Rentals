...
function TenantSubSuccessInner() {
  ...
  useEffect(() => {
    ...
    async function verify() {
      try {
        const res = await fetch('/api/verify-tenant-subscription', {
          method: 'POST',
      ...
      {status === 'success' && (
        <div style={{ textAlign: 'center', maxWidth: 440 }}>
          ...
          <p style={{ color: '#888', fontSize: 14, lineHeight: 1.7, marginBottom: '2rem' }}>
            You can now reveal landlord contact details for free on any listing.
            {expiryDate && (
        ...
        <div style={{ textAlign: 'center', maxWidth: 440 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <a
              ...
              style={{ width: '100%', padding: '14px', background: '#0ef6cc', color: '#080a0f', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: 15, cursor: 'pointer', textAlign: 'center', textDecoration: 'none', display: 'block', boxSizing: 'border-box' }}
            >
              Contact Support
            </a>
            <button
...
export default function TenantSubSuccessPage() {
  return (
